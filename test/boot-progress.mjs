...
/* Dates here have to be relative to today, never absolute. STALE_DAYS is 10, so
   a session pinned to a calendar date silently crosses the lay-off threshold as
   real time passes - and sooner on a runner in UTC than on a machine in PDT,
   which is exactly how this suite started failing on CI while passing at home.
   Same rule as todayISO(): local date arithmetic, never toISOString(). */
const daysAgo = n => {
...
seed(daysAgo(4), "off-squat", "Off Squat", "strength", 3, { weight: 200, reps: 5 });
...
seed(daysAgo(4), "back-squat", "Back Squat", "strength", 3, { weight: 200, reps: 5 });
let sg = lb.suggestNext(step({ exerciseId: "back-squat", name: "Back Squat", sets: 3, reps: 5, weight: 200 }));
check("it read the last session", sg.date, daysAgo(4));
...
seed(daysAgo(4), "bench-press-b", "Bench Press", "strength", 3, { weight: 150, reps: 5 });
...
seed(daysAgo(8), "front-squat", "Front Squat", "strength", 3, { weight: 185, reps: 3 });
...
seed(daysAgo(7), "front-squat", "Front Squat", "strength", 3, { weight: 185, reps: 4 });
sg = lb.suggestNext(step({ exerciseId: "front-squat", name: "Front Squat", sets: 3, reps: 5, weight: 185 }));
...
seed(daysAgo(6), "front-squat", "Front Squat", "strength", 3, { weight: 165, reps: 5 });
sg = lb.suggestNext(step({ exerciseId: "front-squat", name: "Front Squat", sets: 3, reps: 5, weight: 185 }));
check("after a deload it ramps, not jumps", sg.weight, 175);
check("rather than back onto the failure weight", sg.weight < 185, "true");
seed(daysAgo(5), "front-squat", "Front Squat", "strength", 3, { weight: 175, reps: 5 });
sg = lb.suggestNext(step({ exerciseId: "front-squat", name: "Front Squat", sets: 3, reps: 5, weight: 185 }));
check("and keeps ramping the next session", sg.weight, 185);

/* --- bodyweight --- */
seed(daysAgo(4), "archer-push-up", "Archer Push-up", "bodyweight", 3, { reps: 10, added: null });
...
seed(daysAgo(4), "weighted-dip", "Weighted Dip", "bodyweight", 3, { reps: 6, added: 25 });
...
/* --- holds and cardio --- */
seed(daysAgo(4), "copenhagen-hold", "Copenhagen Hold", "hold", 3, { secs: 40 });
...
seed(daysAgo(4), "trail-run", "Trail Run", "cardio", 1, { duration: 30, distance: 5 });
...
seed(daysAgo(4), "both-squat", "Both Squat", "strength", 3, { weight: 300, reps: 3 });
...
check("today's sets are ignored in favour of the real last session", both.date, daysAgo(4));
